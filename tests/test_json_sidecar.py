...
def make_json_drop(tmp_path: Path) -> Path:
...
@pytest.mark.asyncio
async def test_json_drop_publishes_end_to_end(tmp_path: Path):
    md = make_json_drop(tmp_path)
    ...
    ig_error = out["published"]["instagram"]["error"].lower()
    assert "not authenticated" in ig_error or "facebook" in ig_error
