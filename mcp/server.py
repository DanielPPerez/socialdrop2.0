...
@mcp.tool()
def list_drops(folder: str = ".") -> list[DropInfo]:
...
@mcp.tool()
def publish(folder: str = ".", only: list[str] | None = None, now: bool = False) -> dict:
    ...
    results = publish_folder(_folder(folder), only=only, respect_schedule=not now)
    return {k: v for k, v in results.items()}


@mcp.tool()
def get_stats(folder: str = ".") -> dict:
